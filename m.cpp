...
{
    auto &irb = *mCurIrb;
    // 将 ASG 中的数组类型转换为 LLVM 的 ArrayType
    auto *arrTy = llvm::cast<llvm::ArrayType>(self(obj->type));
    // 分配临时内存用于存储初始化值
    llvm::Value *tmpPtr = irb.CreateAlloca(arrTy, nullptr, "initArray");

    // 如果初始化列表为空或第一项为隐式初始化（空数组初始化），返回零值
    if (obj->list.empty() || (!obj->list.empty() && obj->list[0]->dcst<ImplicitInitExpr>()))
        return llvm::Constant::getNullValue(arrTy);

    // 定义递归 lambda 用于处理 InitListExpr 的初始化
    std::function<void(llvm::Value *, llvm::ArrayType *, asg::InitListExpr *, std::vector<llvm::Value *> &)> storeRec =
        [&](llvm::Value *basePtr, llvm::ArrayType *curTy, asg::InitListExpr *expr, std::vector<llvm::Value *> &idx)
    {
        for (size_t i = 0; i < expr->list.size(); ++i)
        {
            idx.push_back(irb.getInt64(i));
            // 如果当前子表达式本身是一个 InitListExpr，且当前数组元素类型为数组，则递归处理
            if (auto subList = expr->list[i]->dcst<InitListExpr>())
            {
                if (auto innerTy = llvm::dyn_cast<llvm::ArrayType>(curTy->getElementType()))
                {
                    storeRec(basePtr, innerTy, subList, idx);
                }
                else
                {
                    // 非数组类型则直接处理（一般不会走到这里）
                    llvm::Value *elemPtr = irb.CreateInBoundsGEP(curTy, basePtr, idx, "elemPtr");
                    llvm::Value *initVal = self(expr->list[i]);
                    irb.CreateStore(initVal, elemPtr);
                }
            }
            else
            {
    ...
    // 初始索引：第一个索引为 0，用于 alloca 得到的指针（基地址）
    std::vector<llvm::Value *> indices;
    indices.push_back(irb.getInt64(0));
    storeRec(tmpPtr, arrTy, obj, indices);
    return tmpPtr;
}
...
void EmitIR::trans_init(llvm::Value *val, Expr *obj, llvm::Type *ty)
{
    auto &irb = *mCurIrb;
    ...
    if (auto p = obj->dcst<InitListExpr>())
    {
        ...
        std::vector<llvm::Value *> idxBase;
        idxBase.push_back(irb.getInt64(0));
            ...
            [&](llvm::ArrayType *ty, std::vector<llvm::Value *> idxList)
        {
            ...
            for (int i = 0; i < numElements; i++)
            {
                ...
                {
                    llvm::Value *elemPtr = irb.CreateInBoundsGEP(ty, val, idx, "elemPtr");
                    llvm::Value *initVal = self(p->list[i]);
                    irb.CreateStore(initVal, elemPtr);
                }
            }
        };
...
void EmitIR::operator()(VarDecl *obj) // 考虑全局变量和局部变量，不能用islocal判断
{
    ...
    { // 局部变量
        ...
        trans_init(alloca, obj->init, ty); // 把变量类型直接传上去
    }
    else
    {                                           // 全局变量
...
void EmitIR::operator()(FunctionDecl *obj)
{
    ...
    {
        auto block = exitIrb.GetInsertBlock();
